...
void MyGLWidget::ini_camera(){
  ...
  znear = radiEsc;
  zfar = 3.0*radiEsc;
  left = bottom = leftaux = bottomaux = -radiEsc;
  right = top = rightaux = topaux = radiEsc;
  projectTransform();
...
void MyGLWidget::calculaRadi(){
  float miny = 0.0;//std::min(0.0f, minymodel); //en aquest cas es poden calcular a ma, ni no sabem els valors calculaCapsaModel
  float maxy = 1.0;//std::max(0.0f, maxymodel);
  float minz = -2.0;//std::min(0.0f, minzmodel);
  float maxz = 2.0;//std::max(0.0f, maxzmodel);
  float minx = -2.0;//std::min(0.0f, minxmodel);
  float maxx = 2.0;//std::max(0.0f, maxxmodel);
