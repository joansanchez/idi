...
MyGLWidget::MyGLWidget (QWidget* parent) : QOpenGLWidget(parent)
{
  ...
  movimentX = 0.0;
  tipusCamera = false; //false = general, true = només patricio
  colorMaterial = glm::vec3(0,0,0);
}
...
void MyGLWidget::initializeGL ()
{
  ...
  matriuaux = glm::vec3 (-2,1,0);
  glUniform3fv(posFocus,1,&matriuaux[0]);
}
...
void MyGLWidget::createBuffers ()
{
  ...
  glm::vec3 diff = colorMaterial;
  //std::cout<< "vector difusa: "<<diff[0]<<", "<<diff[1]<<", "<<diff[2]<<std::endl;
  glm::vec3 spec(0,0,0);
  float shin = 100;
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, glm::vec3(movimentX,0,0));
  TG = glm::translate(TG, glm::vec3(-2,0,0));
  TG = glm::rotate(TG, float(90.0/180.0 * M_PI), glm::vec3(0.0, 1.0, 0.0));
...
void MyGLWidget::modelTransformPatricio2 ()
{
  ...
  TG = glm::translate(TG, glm::vec3(0,1.25,0));
  TG = glm::rotate(TG, float(-90.0/180.0 * M_PI), glm::vec3(0.0, 1.0, 0.0));
...
void MyGLWidget::ini_camera(){
  if (tipusCamera == false){
    distancia = 2*radiEsc;
    FOV = FOVini = (float)2*asin(radiEsc/distancia);
    ra = 1.0f;
    znear = radiEsc;
    zfar = 3.0*radiEsc;
    projectTransform();
    ...
    FOV = FOVini = (float)M_PI/2.0;
    ra = 1.0f;
    znear = 0.5;
    zfar = 3.0*radiEsc;
    projectTransform();
    OBS = glm::vec3(matriuaux[0], 0.5, matriuaux[2]);
    VRP = glm::vec3(2, 0.25,1);
    UP = glm::vec3(0, 1, 0);
    viewTransform();
  }
}
...
void MyGLWidget::calculaRadi(){
  ...
  int minx = -2;
  int maxx = 2;
  glm::vec3 centre = glm::vec3((maxx+minx)/2.0,(maxy+miny)/2.0,(maxz+minz)/2.0);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_A: { // moviment eix de les X+
      movimentX += 0.1;
      matriuaux[0] += 0.1;
      glUniform3fv(posFocus,1,&matriuaux[0]);
      ini_camera();
    ...
    case Qt::Key_D: { // moviment eix de les X-
      movimentX -= 0.1;
      matriuaux[0] -= 0.1;
      glUniform3fv(posFocus,1,&matriuaux[0]);
      ini_camera();
    ...
    case Qt::Key_C: { // moviment eix de les X-
      if (tipusCamera == false){ //canvi a càmara pàtricio
        tipusCamera = true;
      }
      else { //canvi a càmera general
        tipusCamera = false;
      }
      ini_camera();
      std::cout<<"canvi camera des de teclat" <<std::endl;
      emit(canviCameraPrograma());
      break;
    }
...
void MyGLWidget::canviarCamera(){
  makeCurrent();
  std::cout<<"canvi camera des de check box"<<std::endl;
  if (tipusCamera == false){ //canviem a camera Patricio
    tipusCamera = true;
    ini_camera();
  }else { //canviem a general
    tipusCamera = false;
    ini_camera();
  }
...
void MyGLWidget::setred(int red){
  makeCurrent();
  //std::cout<<" material red: "<<red<<" transfo: " <<red/100.0<<std::endl;
  colorMaterial[0] = (float)red/100.0;
  createBuffers();
  update();
}
...
void MyGLWidget::setgreen(int green){
  makeCurrent();
  colorMaterial[1] = (float)green/100;
  createBuffers();
  update();
}
...
void MyGLWidget::setblue(int blue){
  makeCurrent();
  colorMaterial[2] = (float)blue/100;
  createBuffers();
  update();
}
