...
void MyGLWidget::carregaTerra(){
  ...
  Colors[1] = glm::vec3(0.0, 0.0, 0.0);
  Colors[2] = glm::vec3(0.0, 0.0, 0.0);
...
void MyGLWidget::canviarterra(int red, int green, int blue){
  makeCurrent();
  std::cout << "canviarterra" << std::endl;
  float redf = (float)red;
  float greenf = (float)green;
  float bluef = (float)blue;

  glm::vec3 Colors[4];  // Tres vèrtexs amb X, Y i Z
  Colors[0] = glm::vec3(redf, greenf, bluef);
  Colors[1] = glm::vec3(redf, greenf, bluef);
  Colors[2] = glm::vec3(redf, greenf, bluef);
  Colors[3] = glm::vec3(redf, greenf, bluef);
